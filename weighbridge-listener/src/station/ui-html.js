...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  input { width:100%; min-height:50px; padding:0 14px; background:var(--surface-3); border:1.5px solid var(--border); border-radius:var(--btn-radius); color:var(--text); font-family:var(--font-body); font-size:15px; outline:none; transition:border-color .15s, box-shadow .15s; }
  input::placeholder { color:var(--text-3); }
  input:focus { border-color:var(--brand); box-shadow:0 0 0 3px rgba(60,131,194,.22); }
...
<main>
  ...
  <section class="card">
    ...
    <label>Nama Barang</label><input id="namaBarang" value="BATU BARA" />
    <div class="row">
      <div><label>Supplier</label><input id="supplier" placeholder="MM TALENTA" /></div>
      <div><label>No. PO / DO</label><input id="noPoDo" placeholder="-" /></div>
    </div>
    <label>Keterangan</label><input id="keterangan" placeholder="SOLAR FULL" />
    <div class="row">
...
var FIELD_KEYS = ['namaBarang', 'supplier', 'noPoDo', 'keterangan', 'operator', 'supir'];
var DEFAULTS = { namaBarang: 'BATU BARA' };
...
function resetForm() {
  ...
  $('supplier').value = ''; $('noPoDo').value = ''; $('keterangan').value = '';
...
function loadEntryIntoForm(entry) {
  ...
  for (var i = 0; i < FIELD_KEYS.length; i++) {
    var k = FIELD_KEYS[i];
    $(k).value = entry[k] || (k === 'namaBarang' ? DEFAULTS.namaBarang : '');
  }
...
async function poll() {
  try {
    ...
    var plate = $('noPolisi').value.trim();
    $('btnWeigh').disabled = !settled || !plate || form.hasTotals;
