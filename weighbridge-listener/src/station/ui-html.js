...
function updateHint() {
  ...
  else if (form.nextWeighing === 1) hint = 'Tekan Timbang untuk Timbangan #1.';
  else hint = 'Tekan Timbang untuk Timbangan #2.';
...
async function fetchHistory() {
  try {
    ...
    var d = await r.json();
    if (!r.ok) { $('historyTotals').textContent = 'Gagal memuat: ' + (d.error || r.status); return; }
    renderHistory(d.tickets);
    var t = d.totals || {};
    $('historyTotals').textContent = date
      ? (t.count + ' truk · Gross ' + fmt(t.gross) + ' Kg · Netto ' + fmt(t.netto) + ' Kg')
      : '';
  } catch (e) { $('historyTotals').textContent = 'Gagal memuat (offline?).'; }
}
...
async function poll() {
  try {
    ...
    else if (s.stable) { pill.className = 'status-pill pill stable'; $('pillText').textContent = 'STABIL ✓'; }
    else { pill.className = 'status-pill pill moving'; $('pillText').textContent = 'Bergerak (US)'; }

    $('noTiket').value = s.nextTiket || '';
    var plate = $('noPolisi').value.trim();
    $('btnWeigh').disabled = !s.connected || !plate || form.hasTotals || missingRequired();
