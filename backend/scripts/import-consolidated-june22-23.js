...
function parseJune22(ws) {
  ...
  ws.eachRow((row, rn) => {
    ...
    const grossSiteRaw  = row.getCell(5).value;
    const grossSite     = Number(grossSiteRaw?.result ?? grossSiteRaw) || 0;
    const tareSiteRaw   = row.getCell(6).value;
    const tareSite      = Number(tareSiteRaw?.result ?? tareSiteRaw) || 0;
    ...
    const grossJettyRaw = row.getCell(10).value;
    const grossJetty    = Number(grossJettyRaw?.result ?? grossJettyRaw) || 0;
    const tareJettyRaw  = row.getCell(11).value;
    const tareJetty     = Number(tareJettyRaw?.result ?? tareJettyRaw) || 0;
